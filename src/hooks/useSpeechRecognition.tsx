...
export const useSpeechRecognition = (): UseSpeechRecognitionReturn => {
  ...
  const reconnectTranscriber = useCallback(async () => {
    console.log('Attempting to reconnect transcriber...');
    try {
  ...
  // Update the transcriber event handlers
  const setupTranscriberEvents = (transcriber: RealtimeTranscriber) => {
    ...
    transcriber.on('error', (error) => {
      ...
      if (error.message.includes('Session idle') || error.message.includes('Session expired')) {
        reconnectTranscriber();
      }
    });
  ...
  // Update initializeTranscriber to use the new event setup
  const initializeTranscriber = async () => {
    try {
      ...
      setupTranscriberEvents(transcriber);
  ...
  // Add a cleanup interval to prevent idle timeouts
  useEffect(() => {
    let keepaliveInterval: NodeJS.Timeout | null = null;

    if (isRecording && transcriberRef.current) {
      // Send a small audio packet every 25 seconds to keep the connection alive
      keepaliveInterval = setInterval(async () => {
        if (transcriberRef.current && isRecordingRef.current) {
          try {
            // Send a minimal buffer to keep the connection alive
            const keepaliveBuffer = new Int16Array(160).fill(0);
            await transcriberRef.current.sendAudio(keepaliveBuffer.buffer);
          } catch (err) {
            console.error('Error sending keepalive:', err instanceof Error ? err.message : 'Unknown error');
          }
        }
      }, 25000);
    }
  ...
  const setupAudioProcessing = async (stream: MediaStream) => {
    ...
    try {
      // Load and register the audio worklet with the correct path
      ...
      throw new Error(`Failed to load audio worklet: ${errorMessage}`);
    }
  };
  ...
  const handleAudioData = async (audioData: Int16Array, bufferInfo: any) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('Socket is not open')) {
        setIsSocketConnected(false);
      }
    }
  };
